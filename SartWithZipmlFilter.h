...
#ifndef rtkSartWithZipmlFilter_h
#define rtkSartWithZipmlFilter_h
 ...
 * \test rtksarttest.cxx
 *
 * \author Simon Rit, (added zipml )
 *
 * \ingroup ReconstructionAlgorithm
 */
template<class TInputImage, class TOutputImage=TInputImage>
class ITK_EXPORT SartWithZipmlFilter :
  public rtk::IterativeConeBeamReconstructionFilter<TInputImage, TOutputImage>
{
public:
  /** Standard class typedefs. */
  typedef SartWithZipmlFilter                                 Self;
  ...
  typedef itk::QuantizationFilter<InputImageType>											QuantizationFilterProjectionType;
  typedef itk::QuantizationFilter<OutputImageType>											QuantizationFilterVolumeType;
  ...
  typedef itk::MinimumMaximumImageCalculator<OutputImageType>	MinMaxCalcType;

/** Standard New method. */
  itkNewMacro(Self);

  /** Runtime information support. */
  itkTypeMacro(SartWithZipmlFilter, IterativeConeBeamReconstructionFilter);
  ...
  itkGetMacro(MyVerbose, bool);// off by defaults
  itkSetMacro(MyVerbose, bool);

  //void SetNumberOfBits(std::vector<int>& NumberOfBits_arg);
  ...
  itkGetMacro(NumberOfBitsQ00, int);
  itkSetMacro(NumberOfBitsQ00, int);
  
  itkGetMacro(NumberOfBitsQ01, int);
  itkSetMacro(NumberOfBitsQ01, int);
  
  itkGetMacro(NumberOfBitsQ02, int);
  itkSetMacro(NumberOfBitsQ02, int);
  
  itkGetMacro(NumberOfBitsQ03, int);
  itkSetMacro(NumberOfBitsQ03, int);
  
  itkGetMacro(NumberOfBitsQ04, int);
  itkSetMacro(NumberOfBitsQ04, int);
  
  itkGetMacro(NumberOfBitsQ05, int);
  itkSetMacro(NumberOfBitsQ05, int);
  
  itkGetMacro(NumberOfBitsQ06, int);
  itkSetMacro(NumberOfBitsQ06, int);
  
  itkGetMacro(NumberOfBitsQ07, int);
  itkSetMacro(NumberOfBitsQ07, int);
...
protected:
  SartWithZipmlFilter();
  ~SartWithZipmlFilter() ITK_OVERRIDE {}
  ...
  typename QuantizationFilterProjectionType::Pointer		m_QuantRBIFilter;				//[0]
  typename QuantizationFilterProjectionType::Pointer		m_QuantProjExtractionFilter;	//[0]
  typename QuantizationFilterVolumeType::Pointer			m_QuantFPInputFilter;			//[0]
  typename QuantizationFilterProjectionType::Pointer		m_QuantFPResultFilter;			//[0]
  typename QuantizationFilterProjectionType::Pointer		m_QuantDiffResultFilter;		//[0]	
  typename QuantizationFilterProjectionType::Pointer		m_QuantB4NormFilter;			//[0]
  typename QuantizationFilterProjectionType::Pointer		m_QuantBPInput;					//[0]
  typename QuantizationFilterProjectionType::Pointer 		m_QuantAdderInput;				//[0]	
  //typename QuantizationFilterProjectionType::Pointer		m_Qua
  typename ImageFileWriterType::Pointer						m_MyWriter01;
  typename ImageFileWriterType::Pointer						m_MyWriter02;

  typename SubtractImageFilterType::Pointer		m_Subtractor;// = SubtractImageFilterType::New();
  typename MinMaxCalcType::Pointer				m_MinMaxCalculator;// = MinMaxCalcType::New();
...
private:
  ...
  SartWithZipmlFilter(const Self&);
  ...
  bool m_MyVerbose;
  bool m_Quantization;
 // std::vector<int> m_NumberOfBits;
  /**/
	int m_NumberOfBitsQ00;
	int m_NumberOfBitsQ01;
	int m_NumberOfBitsQ02;
	int m_NumberOfBitsQ03;
	int m_NumberOfBitsQ04;
	int m_NumberOfBitsQ05;
	int m_NumberOfBitsQ06;
	int m_NumberOfBitsQ07;
...
#include "SartWithZipmlFilter.hxx"
#endif
